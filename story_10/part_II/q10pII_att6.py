def simulate_dragon_sheep_game(board_str, rounds=20, debug=False):
    """
    ...
    for r, line in enumerate(lines):
        for c, char in enumerate(line):
            if char == 'D':
                ...
                initial_sheep.add((r, c))
            elif char == '#':
                hideouts.add((r, c))
    
    # Track all possible dragon positions (we don't need counts, just positions)
    dragon_positions = {dragon_pos}
    sheep_positions = initial_sheep.copy()
    total_eaten = 0
    ...
    for round_num in range(1, rounds + 1):
        ...
        for dr, dc in dragon_positions:
            for dmr, dmc in dragon_moves:
                ...
                if 0 <= new_r < height and 0 <= new_c < width:
                    new_dragon_positions.add((new_r, new_c))
        
        dragon_positions = new_dragon_positions
        
        if debug:
            print(f"\n=== Round {round_num} ===")
            print(f"Dragon can be at {len(dragon_positions)} positions")
        
        # Sheep turn: move all sheep down one row FIRST
        ...
        if debug:
            print(f"Sheep remaining: {len(sheep_positions)}")
        
        # Now count how many sheep can be eaten
        # A sheep can be eaten if it's on a position where dragon can be AND not on hideout
        eaten_this_round = 0
        sheep_to_remove = set()
        
        for sheep_pos in sheep_positions:
            if sheep_pos in dragon_positions:
                # Check if it's a hideout
                if sheep_pos not in hideouts:
                    eaten_this_round += 1
                    sheep_to_remove.add(sheep_pos)
        
        # Remove eaten sheep
        sheep_positions -= sheep_to_remove
        total_eaten += eaten_this_round
        
        if debug:
            print(f"Eaten this round: {eaten_this_round}")
            print(f"Total eaten so far: {total_eaten}")
...
print("Testing with example (3 rounds):")
result_3 = simulate_dragon_sheep_game(example_board, rounds=3, debug=True)
print(f"\n✓ Result: {result_3} sheep eaten (expected: 27)")

print("\n" + "="*50)

# Full simulation with 20 rounds
result_20 = simulate_dragon_sheep_game(example_board, rounds=20)
print(f"\n✓ Example with 20 rounds: {result_20} sheep eaten")

print("\n" + "="*50)

# ========================================
# PASTE YOUR PUZZLE INPUT HERE (between the triple quotes)
# ========================================
your_input = """
...
# Calculate answer for your input
if your_input.strip():
    answer = simulate_dragon_sheep_game(your_input, rounds=20)
    print(f"\n🎯 ANSWER: {answer} sheep eaten in 20 rounds")
else:
    print("\n⚠️ Please paste your input above!")
