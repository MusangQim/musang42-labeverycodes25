...
if not dragon_pos:
    ...
    eaten_sheep_global = set()  # Sheep eaten across all paths
    total_sheep = 0
        ...
        for r, c in current_positions:
            for dr, dc in directions:
                ...
                if 0 <= nr < rows and 0 <= nc < cols and grid[nr][nc] != '#':
                    ...
                    # If there's a sheep here and not eaten yet
                    if grid[nr][nc] == 'S' and (nr, nc) not in eaten_sheep_global:
                        sheep_this_round.add((nr, nc))
        
        # Add newly eaten sheep to global set
        eaten_sheep_global.update(sheep_this_round)
        
        round_sheep_count = len(sheep_this_round)
        total_sheep += round_sheep_count
        print(f"Round {round_num}: {round_sheep_count} sheep, Total: {total_sheep}")
